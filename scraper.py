...
def discover_trending_keywords(scraper_key, progress_callback=None):
    if progress_callback:
        progress_callback(0.02, "Scan de la page d'accueil Etsy...")
    ...
    title_bigram_freq = defaultdict(int)

    for pi, page_url in enumerate(pages_to_scan):
        if progress_callback:
            progress_callback(0.02 + 0.07 * ((pi + 1) / len(pages_to_scan)),
                            f"Scan page {pi+1}/{len(pages_to_scan)}...")

        html, error = fetch_via_scraperapi(page_url, scraper_key)
        if error:
            last_error = error
            continue
        ...
        for link in listing_links:
            ...
            if title:
                ...
                for i in range(len(words) - 1):
                    ...
                    title_bigram_freq[bigram] += 1

        time.sleep(0.5)

    top_bigrams = sorted(title_bigram_freq.items(), key=lambda x: x[1], reverse=True)
...
def analyze_keywords_auto(scraper_key, progress_callback=None):
    ...
    if progress_callback:
        progress_callback(0.12, f"{len(trending)} mots-cles a analyser...")

    results = []
    for i, kw in enumerate(trending):
        pct = 0.12 + (0.85 * (i + 1) / len(trending))
        if progress_callback:
            progress_callback(pct, f"Analyse: {kw} ({i+1}/{len(trending)})")

        data, _ = analyze_keyword_quick(kw, scraper_key)
